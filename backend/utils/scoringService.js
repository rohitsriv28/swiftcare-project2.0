...
import { MAX_EXPECTED_APPOINTMENTS, MAX_EXPECTED_REVENUE } from "../config/constants.js";
...
export const calculatePerformanceScore = async (doctorId) => {
  try {
    ...
    const appointmentScore = Math.min(35, (completedAppointments / MAX_EXPECTED_APPOINTMENTS) * 35);
    ...
    const totalScore = Number((appointmentScore + revenueScore + ratingScore).toFixed(1));
...
export const calculateCancellationRisk = async (appointmentInput) => {
  try {
    ...
    const isId = typeof appointmentInput === 'string' || appointmentInput instanceof String || (appointmentInput && appointmentInput.toString && appointmentInput.toString().length === 24 && !appointmentInput.userId);
    
    const appointment = isId ? await appointmentModel.findById(appointmentInput) : appointmentInput;
    if (!appointment) {
    ...
    const slotDateObj = new Date(parseInt(parts[2]), parseInt(parts[1]) - 1, parseInt(parts[0]));
    
    const daysBetween = (slotDateObj.getTime() - bookingDate.getTime()) / (1000 * 3600 * 24);
    ...
    const pastAppointments = await appointmentModel.find({ userId: appointment.userId });
    const pastCancellations = pastAppointments.filter(app => app.isCancelled).length;
