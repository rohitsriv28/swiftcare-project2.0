        ...
        <div className="min-w-[800px]">
          <div className="flex">
            ...
            <div className="flex-1 flex justify-between px-2 text-xs text-gray-500 pb-2">
              {hoursOfDay.map(h => (
                <div key={h} className="w-8 text-center">{h}:00</div>
              ))}
            </div>
          </div>
                ...
                <div className="flex-1 flex justify-between px-2 gap-1">
                  {hoursOfDay.map(hIdx => {
                    ...
                    return (
                      ...
                      >
                        {val > 0 && <span className="text-[10px] text-white font-bold opacity-0 hover:opacity-100">{val}</span>}
                      </div>
                    );
                  })}
                </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">Completed and paid appointments over time.</p>
          </div>
          ...
          {revenueData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={revenueData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{fill: '#6b7280', fontSize: 12}} dx={-10} tickFormatter={(val) => `$${val}`} />
                ...
                <Line yAxisId="left" type="monotone" dataKey="revenue" name="Revenue" stroke="#1A6E80" strokeWidth={3} dot={{r: 4, strokeWidth: 2}} activeDot={{r: 6}} />
                <Line yAxisId="right" type="monotone" dataKey="appointments" name="Appointments" stroke="#E69B0F" strokeWidth={3} dot={{r: 4, strokeWidth: 2}} activeDot={{r: 6}} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-full flex items-center justify-center text-gray-500">
              <Calendar className="mr-2 opacity-50" /> No data available for this period.
            </div>
          )}
        </div>
      </div>
